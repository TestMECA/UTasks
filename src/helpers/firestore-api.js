...
import { doc, collection, query, where, getDocs, addDoc, deleteDoc, updateDoc } from 'firebase/firestore/lite';
...
export const getProjectTasks = async (selectedProject) => {
    ...
    tasksQuery = (selectedProject && !collatedTasksExist(selectedProject)) ?
        (
            tasksQuery = query(collection(db, "tasks"), where("userId", "==", auth.currentUser.uid), where(
            ...
            ? (
                ...
                ? (
                    tasksQuery = query(collection(db, "tasks"), where("userId", "==", auth.currentUser.uid), where('date', '==', ''))
